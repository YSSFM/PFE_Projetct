# -------------------------------------------------------------------
# SERVICE DE RECONNAISSANCE FACIALE AVEC TENSORFLOW + MTCNN
# -------------------------------------------------------------------
...
class MobileNetFaceRecognizer:
    """
    Reconnaissance faciale avec MobileNetV2 (TensorFlow) + MTCNN
    """
    ...
    def initialize(self):
        """
        Initialise MTCNN et MobileNetV2
        """
        print("=" * 60)
        print("🚀 INITIALISATION - TENSORFLOW + MOBILENETV2")
        print("=" * 60)
        
        # 1. Détecteur MTCNN (basé sur TensorFlow)
        try:
            self.detector = MTCNN()
            print("✅ Détecteur MTCNN initialisé")
        except Exception as e:
            print(f"❌ Erreur MTCNN: {e}")
            self.detector = None
        
        # 2. Modèle MobileNetV2 (Transfer Learning)
    ...
    def extract_face(self, image, box, target_size=(INPUT_SIZE, INPUT_SIZE)):
        """
